...
Estudiantes e instructores de cursos culturales
...
```env
# MongoDB
MONGODB_URI=mongodb://localhost:27017/educultura_db
...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/educultura_db
...
El servidor estará disponible en: `http://localhost:3019`
...
## 📸 Crear CursoCulturals con Imágenes

### ⚠️ IMPORTANTE: La imagen es OPCIONAL

La imagen **NO es requerida** al crear un cursocultural. Puedes:
- ✅ Crear el cursocultural SIN imagen
- ✅ Agregar la imagen DESPUÉS usando el endpoint de upload

### 🎯 Flujo Recomendado (Paso a Paso)

#### Paso 1: Crear el CursoCultural sin imagen

```bash
POST /api/cursocultural
Authorization: Bearer {{token}}
Content-Type: application/json

{
  "nombre": "CursoCultural de ejemplo",
  "descripcion": "Descripción del cursocultural",
  "disponible": true
}
```
...
**💡 Nota:** Guarda el `_id` del cursocultural creado, lo necesitarás para subir la imagen.

---

#### Paso 2: Subir imagen al CursoCultural

```bash
POST /api/cursocultural/507f1f77bcf86cd799439011/upload-image
Authorization: Bearer {{token}}
...
8. **Copiar** el `_id` del cursocultural creado

---

#### Paso 2: Subir Imagen

1. **Ir a:** `Upload por Entidad` → `Upload CursoCultural Image`
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3019/api/cursocultural/507f1f77bcf86cd799439011/upload-image
   ```
...
**✅ ¡Listo!** Tu cursocultural ahora tiene imagen.
...
**Imagen original:**
```
http://localhost:3019/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3019/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
TOKEN=$(curl -X POST http://localhost:3019/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"productor@example.com","password":"password123"}' \
  | jq -r '.access_token')

# 2. Crear CursoCultural
PRODUCTO_ID=$(curl -X POST http://localhost:3019/api/cursocultural \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{   "nombre": "CursoCultural de ejemplo",   "descripcion": "Descripción del cursocultural",   "disponible": true }' \
  | jq -r '.data._id')

echo "CursoCultural creado con ID: $PRODUCTO_ID"

# 3. Subir imagen
curl -X POST http://localhost:3019/api/cursocultural/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3019/uploads/`
...
```bash
curl -X GET http://localhost:3019/api/estudiante-profile/me \
...
```bash
curl -X PUT http://localhost:3019/api/estudiante-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `educultura-api.postman_collection.json`
...
├── educultura-api.postman_collection.json
└── package.json
```
